...
<section id="page7" class="page parallax bg6">
  ...
  <div class="cancel-reservation-wrapper" style="margin-top: 50px; text-align: center; color: antiquewhite;">
    ...
    <form action="cancel_reservation.php" method="POST" style="display: inline-block;" onsubmit="return confirm('Are you sure you want to cancel this reservation?');">
        <input type="text" name="reservation_code" placeholder="e.g. GAMA-K27A" required
...
if (isset($_GET['status']) && $_GET['status'] == 'success' && isset($_GET['code'])) {
    $kode_user = htmlspecialchars($_GET['code']);
?>
    <div id="customAlertModal" class="gama-modal-overlay">
        <div class="gama-modal-box">
            <div class="gama-modal-icon">🎉</div>
            <h2>Reservation Successful!</h2>
            <p>Your table has been securely booked.</p>
            
            <div class="gama-code-box">
                <span class="gama-code-title">YOUR RESERVATION CODE</span>
                <span class="gama-code-value"><?= $kode_user; ?></span>
            </div>
            
            <p class="gama-modal-note">⚠️ Please SAVE or SCREENSHOT this code to cancel or change your reservation later.</p>
            
            <button onclick="closeGamaModal()" class="gama-modal-btn">Great, Thank You</button>
        </div>
    </div>
